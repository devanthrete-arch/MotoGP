...
| Production URL | https://moto-gp-chi.vercel.app | also `defaultShareOrigin` in `src/app/sharing/share.ts` and the `canonical` in `index.html` |
...
| Document vault | the user's browser only (`localStorage`) | `src/app/screens/DocVault.tsx` stores file *names/sizes*; documents never leave the device |
...
Nothing in this repo is a secret today: `src/infrastructure/supabase/client.ts` and `api/og.js` both
...
| --- | --- | --- | --- | --- |
| `VITE_SUPABASE_URL` | public | Vercel env vars (all environments); falls back to a literal in `src/infrastructure/supabase/client.ts` | browser bundle, `api/og.js` | only when the project moves |
...
2. **Feed query latency.** `listHostedPostsPage` (`src/infrastructure/hosted/community.ts`) is
...
4. **Sync failure rate from `HostedResult`.** `src/infrastructure/hosted/kernel/result.ts` never
...
- **document vault contents or filenames** - `src/app/screens/DocVault.tsx` keeps RC,
