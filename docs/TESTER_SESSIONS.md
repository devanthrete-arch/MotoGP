...
| Broken interaction, lost local state, a fallback that didn't fire, a share that produced a dead link | **Frontend engineer** | Fixed in the owning module, covered by a unit test (e.g. `src/share.test.ts` for share and deep links), and verified on a real Android device. |
