...
3. Run `npm run test`.
4. Confirm the feed, filters, saved posts, following mode, city circles, ownership playbooks, post detail quality meter, buyer shortlist, profile form, comments, reports, moderator queue, write form, subscriptions, model notebooks, post sharing, notebook sharing, garage export, garage timeline, garage insights, build loop, and feedback lane work.
